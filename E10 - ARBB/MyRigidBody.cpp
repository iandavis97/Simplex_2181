...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	v3Corner[5] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	v3Corner[6] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);
	v3Corner[7] = m_v3MaxL;

	//placing squares in world space
	for (uint uIndex = 0; uIndex < 8; ++uIndex)
	{
		v3Corner[uIndex] = vector3(m_m4ToWorld*vector4(v3Corner[uIndex],1.0f));
	}

	//identify min & max as first corner
	m_v3MaxG = m_v3MinG = v3Corner[0];

	//get new min & max for global space
	for (uint i = 1; i < 8; ++i)
	{
