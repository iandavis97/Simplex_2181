...
{
	//look one by one for specified unique id
...
{
	//create a temporal entity to store object
	MyEntity* pTemp = new MyEntity(a_sFileName, a_sUniqueID);
	//if i was able to generate it add to the list
...
void Simplex::MyEntityManager::RemoveEntity(uint a_uIndex)
{
	...
	if (a_uIndex >= m_uEntityCount)
		a_uIndex = m_uEntityCount - 1;
	//if entity not very last we swap for last
...
String Simplex::MyEntityManager::GetUniqueID(uint a_uIndex)
{
	...
	if (m_entityList.size() == 0)
		return "";
	//if index is larger than number of entries we are asking for last one
...
MyEntity* Simplex::MyEntityManager::GetEntity(uint a_uIndex)
{
	...
	if (m_entityList.size() == 0)
		return nullptr;
	//if index larger than number of entries we are asking for the last one
	if (a_uIndex >= m_entityList.size())
		a_uIndex = m_entityList.size() - 1;
...
{
	//if out of bounds will do for all
	if (a_uIndex >= m_uEntityCount)
	{
		//add for each one in entity list
