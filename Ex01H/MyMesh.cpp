...
void MyMesh::GenerateCone(float a_fRadius, float a_fHeight, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	for (float angle = 0.0f; angle < 2.0f*PI; angle += step)
	{
		...
		point4 = vector3(point1.x, point1.y, point1.z+a_fHeight);
		angle -= step;
...
void MyMesh::GenerateCylinder(float a_fRadius, float a_fHeight, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	for (float angle = 0.0f; angle < 2.0f*PI; angle += step)
	{
		...
		cosine = cos(angle);
		sine = sin(angle);
		point5 = vector3(point4.x + (cosine*a_fRadius), point4.y + (sine*a_fRadius), point4.z + a_fHeight);
		angle += step;
		cosine = cos(angle);
		sine = sin(angle);
		point6 = vector3(point4.x + (cosine*a_fRadius), point4.y + (sine*a_fRadius), point4.z + a_fHeight);
		angle -= step;
